...
| bug07 | Filmes com duração `0` e `-20` foram criados normalmente. | `Conteudo.java`: construtor e setter não validavam `duracaoMinutos`; durante a entrada HTTP, o Jackson ainda encapsulava essa exceção. | Centralizada no setter a rejeição de valores `<= 0` e tratado `HttpMessageNotReadableException`, garantindo HTTP 400 com a mensagem original. | Encapsulamento, validação de estado, desserialização e exceções. |
